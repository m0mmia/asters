...
const {active, transitions, entrance, entranceRepeat} = props;
  const {xStart, xEnd, yStart, yEnd, childTransitions, variant='horizontal'} = transitions
  ...
  useEffect(() => {
    ...
    if(a && b && ((xStart1 && xStart2 && xEnd1, xEnd2 ) || (yStart1 && yStart2 && yEnd1 && yEnd2))) {
        tl = getAnimationAll();
    } else {
  ...
  const doPopup = () => {
    gsap.set(ref.current, {y: ref.current.clientHeight * 2 * 0.80 })
    gsap.to(ref.current, {
      delay: 0.2,
      duration: 0.4,
      opacity: 1,
